...
//		String ext3 = filePath13.substring(index3 + 1).toUpperCase();
//		Image image3 = Image.builder().path(filePath13).type(IMAGE_TYPE.valueOf(ext3)).build();
//		TripProduct product = TripProduct.builder().member(null).title("title").description("desc").price(10000).continent(null)
//				.build();
...
//			e.printStackTrace();
//		}
//		TripProduct searchProduct = productRepository.findById((long) 1).orElseThrow(() -> new Exception("없음"));
...
//
//		Page<TripProduct> page = productRepository.findAll(pageRequest);
...
//		Page<TripProduct> pages= productRepository.findByContinent(continent, pageRequest);
